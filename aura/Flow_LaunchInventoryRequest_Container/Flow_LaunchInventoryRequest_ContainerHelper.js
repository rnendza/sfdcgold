({
    /**
     * Calls the server to the Inventory_Request__c
     * @param cmp
     */
    retrieveIr: function (cmp) {
        let irId = cmp.get('v.selectedRecordId');
        let params = {"irId": irId};
        let action = 'c.retrieveInventoryRequestAndLineItems';
        this.log(cmp,'calling apex retrieveInventoryRequestAndLineItems with params','debug',JSON.stringify(params));
        cmp.lax.enqueue(action, params)
            .then(response => {
               //this.fireIrFlow(cmp, response);
     ...
     */
    fireIrFlow: function (cmp,response) {
        let dto = response;
