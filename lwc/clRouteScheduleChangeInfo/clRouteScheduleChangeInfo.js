...
export default class ClRouteScheduleChangeInfo extends LightningElement {
    ...
    observeUpdatePayload(payload) {
        ...
        if(payload.Ui_Message__c) {
            if (payload.Route_Schedule_Ids__c) {
                let arrIds = payload.Route_Schedule_Ids__c.split(',');
                if (arrIds.includes(this.recordId)) {
                    this.showToast('', payload.Ui_Message__c, 'success');
                    if(payload.Status__c && payload.Status__c == 'Complete') {
                        console.log('fire jobcomplete event');
                        this.dispatchEvent( new CustomEvent('jobcomplete', {detail: payload}) );
                    }
                } else {
                    if(payload.Status__c) {
                        if (payload.Status__c == 'ScheduleCreationComplete') {
                            let msg = payload.Ui_Message__c + ' Please view the new route schedules in the route schedule listview.'
                            this.showToast('', msg, 'success');
                            this.dispatchEvent(new CustomEvent('jobcomplete', {detail: payload}));
                        }
                    }
                }
            } else if (payload.Operation_Type__c && payload.Operation_Type__c == 'MDT_UPDATE') {
                this.showToast('', payload.Ui_Message__c, 'success');
                this.dispatchEvent( new CustomEvent('jobcomplete', {detail: payload}) );
            } else if (payload.Status__c) {
                if (payload.Status__c == 'ScheduleCreationFailed') {
                    this.showToast('', payload.Ui_Message__c, 'error');
                    this.dispatchEvent(new CustomEvent('jobcomplete', {detail: payload}));
                }
            }
        }
