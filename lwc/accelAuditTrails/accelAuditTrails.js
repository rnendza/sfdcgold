...
import retrieveAuditTrails from '@salesforce/apex/AuditTrailController.retrieveAuditTrails';
import {dtHelper} from "./accelAuditTrailsDtHelper";

const GLOBAL_CONSTANTS = getConstants();
const MAP_KEY_AUDIT_TRAIL_WRAPS = 'AUDIT_TRAIL_WRAPS';
const MAX_RESULTS = 600;
const ERROR='error',INFO='info',DEBUG='debug',WARN='warn';

/**
 * AccelAuditTrails will provide a secured user the ability to list all setup audit trail data in the
 * SetupAuditTrail sObject.
 *
 * It will allow a user to filter data via server searches and client side filters. As well as export data.
 */
export default class AccelAuditTrails extends LightningElement {
    ...
    @api showAssistance;
    @api assistanceTitle = 'Audit Trail Assistant'
    @api cardTitle = 'Setup Audit Trails';
    @api cardTitleSearch = 'Audit Trail Search';
    @api btnSearchLabel = 'Audit Trail Search';
    ...
    _displayFilters;
    _usersFilterApplied;
    _sectionsFilterApplied;
    _actionsFilterApplied;
    ...
    constructor() {
        ...
        console.info('%c----> /lwc/accelAuditTrails', GLOBAL_CONSTANTS.CONSOLE_MODULE_STYLE);
    }
    ...
    connectedCallback() {
        ...
        this.doRetrieveAuditTrails(this.searchParams);
     ...
     * @param params  A AuditTrailParams object.
     *
     * @see AuditTrailParams
     */
    doRetrieveAuditTrails( params ) {
        this._isLoading = true;
        this.log(DEBUG,'calling doRetrieveAuditTrails with params:',params);
        retrieveAuditTrails( {params:params} )
            .then((data) => {
                this.log(DEBUG, 'audit trails dto->', data);
                this.dtRecords = uiHelper.getMapValue(MAP_KEY_AUDIT_TRAIL_WRAPS,data.values);
                ...
                if(!data.isSuccess) {
                    this._displayServerSideSearch = true;
                }
            })
            .catch((error) => {
                this.error = error;
                ...
                uiHelper.showToast(this,'','Error loading audit trail records: '+msg,'error');
            })
            .finally(() => {
    ...
    refireQuery() {
        if(this.filterData) {
            ...
            this.searchParams.includeActions = this.filterData.actions;
        ...
        if(!evt.target.value.length) {
            switch (filterId) {
                case 'filterUsers':
                    ...
                    break;
                case 'filterSections':
                    this._sectionsFilterApplied = null;
                    break;
                case 'filterActions':
                    this._actionsFilterApplied = null;
                    break;
            }
            if(this._usersFilterApplied || this._sectionsFilterApplied || this._actionsFilterApplied) {
                this.dtRecordsFiltered = this.dtRecordsFiltered.filter((item) => {
                    return item.createdByFullName.toLowerCase().includes(this._usersFilterApplied.toLowerCase())
                        || item.section.toLowerCase().includes(this._sectionsFilterApplied.toLowerCase())
                        || item.display.toLowerCase().includes(this._actionsFilterApplied.toLowerCase());
    ...
    handleDtSearchKeyChange(event) {
        ...
        switch (filterId) {
            case 'filterUsers':
                filterField = 'createdByFullName';
                this._usersFilterApplied = searchKey;
                break;
            case 'filterSections':
                filterField = 'section';
                this._sectionsFilterApplied = searchKey;
                break;
            case 'filterActions':
                filterField = 'display';
                this._actionsFilterApplied = searchKey;
                break;
        }
        ...
        if(searchKey && searchKey.length > 1){
            try {
                this.dtRecordsFiltered = this.dtRecordsFiltered.filter((item) => {
                    return item.createdByFullName.toLowerCase().includes(searchKey.toLowerCase())
                        || item.section.toLowerCase().includes(searchKey.toLowerCase())
                        || item.display.toLowerCase().includes(searchKey.toLowerCase());
                   // return item[filterField].toLowerCase().includes(searchKey.toLowerCase()) ;
                });
            } catch (e) {
    ...
    generateDefaultFilterSelections() {
        ...
        this.defaultSelections.wmpUsers = [];
        this.defaultSelections.actions = [];
        this.defaultSelections.excludeActions = [];
